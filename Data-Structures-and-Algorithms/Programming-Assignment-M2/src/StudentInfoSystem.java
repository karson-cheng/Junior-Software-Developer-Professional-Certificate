...
public class StudentInfoSystem {
    ...
    public List<Subject> sortSubjectsByName() {
        ...
        for(int i = 0; i < n-1; i++){
            ...
            for(int j = 0; j < n -1 - i; j++){
                Subject currentSubject = (Subject) subjectsByName.get(j);
                Subject nextSubject = (Subject) subjectsByName.get(j+1);

                if(currentSubject.getName().compareTo(nextSubject.getName()) > 0) {
                    subjectsByName.set(j, nextSubject);
                    subjectsByName.set(j + 1, currentSubject);
                    swapped = true;
                }
            }
            if(!swapped)
    ...
    public List<Student> sortStudentsByName(){
        ...
        int n = students.size();
        for(int i = 1; i < n; i++){
            Student unsortStudent = (Student) students.get(i);
            int j = i -1;

            while(j >=0) {
                Student sortStudent = (Student) students.get(j);

                if (sortStudent.getName().compareTo(unsortStudent.getName()) > 0) {
                    students.set(j + 1, sortStudent);
                    j--;
                } else {
                    break;
                }
            }
            students.set(j+1, unsortStudent);
        }
        return students;
    }
